...
function Spinup-AnotherNode {
    ...
    $params = "--networkType regtest -s -cf -p $($dltPort) -a $($apiPort) -i $($IPInterface) -n $($IPInterface):$($DLTStartPort) --walletPassword $($WalletPassword) --disableWebStart --logVerbosity 15"
    if($Worker.IsPresent) {
        $params += " --worker"
    }
            ...
            if($NumInstances -lt 2) {
                ...
                if($gen2Addr -eq $null -or $gen2Addr -eq "") {
                    ...
                    foreach($client in $dstPaths) {
                        ...
                        if($idx -eq 0) {
                            # Genesis node
                            $startParams = "--networkType regtest -s -cf -p $($dltPort) -a $($apiPort) -i $($IPInterface) --genesis 100000000 --genesis2 $($gen2Addr) --walletPassword $($WalletPassword) --disableWebStart --logVerbosity 15"
                        } else {
                            $startParams = "--networkType regtest -s -cf -p $($dltPort) -a $($apiPort) -i $($IPInterface) -n $($IPInterface):$($DLTStartPort) --walletPassword $($WalletPassword) --disableWebStart --logVerbosity 15"
                        }
                        $mining = $true
